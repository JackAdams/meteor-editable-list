...
Template.editable_list_widget.helpers({
  ...
  canEditList : function(data) {
	return EditableList.userCanEdit.call(data,Mongo.Collection.get(data.collection));
  },
...
Template.editable_list_widget.events({
  'click .editable-list li' : function(evt,tmpl) {
	...
	// We need the whole object, not just the _id -- because sometimes requests to edit come from incomplete context situations
	if (EditableList.userCanEdit.call(tmpl.data,Collection)) {
  ...
  this.$('.editable-list').sortable({
	...
	update: function(e,ui){
	  ...
	  Meteor.defer(function() { // Need to defer to avoid too much recursion
  	    EditableText.update.call(self.data,Mongo.Collection.get(self.data.collection),self.data.context,{$set:mod},['beforeReorder','afterReorder'],'reorder list');
...
Template.editable_list_form.events({
  'submit .add-new-list-item, focusout .add-new-list-item input' : function(evt,tmpl) {
	evt.preventDefault();
	if (EditableList.userCanEdit.call(this,Mongo.Collection.get(this.collection))) {
  ...
  'paste .add-new-list-item' : function(evt,tmpl) {
	if (this.allowPasteMultiple && EditableList.userCanEdit.call(this,Mongo.Collection.get(this.collection))) {
